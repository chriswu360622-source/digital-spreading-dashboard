    ...
    <svg viewBox="0 0 ${width} ${height}" role="img">
      ...
      ${config.bars
            .map((bar, j) => {
              ...
              const labelDx = config.barLabelDx?.[j] ?? (config.bars.length > 1 ? (j === 0 ? -7 : 7) : 0);
        ...
        .map((line) => `<polyline points="${points(line)}" fill="none" stroke="${line.color}" stroke-width="3" />
          ${data
            .map((d, i) => {
              ...
              const y = pad.top + plotH - (d[line.key] / pctMax) * plotH;
              const filterKey = config.filterKey ? config.filterKey(d) : d.label;
              const labelDy = line.labelDy ?? (line.key === "utilization" ? 34 : -24);
...
function render() {
  ...
  renderComboChart(el.spreaderChart, aggregateSpreader(values.spreaderRecords), {
    ...
    lines: [{ key: "pct", label: "EFF %", color: "var(--red)", labelDy: -16 }],
  });
  renderComboChart(el.machineChart, aggregateMachine(values.machine), {
    ...
    lines: [
      { key: "completion", label: "Output completion", color: "var(--orange)", labelDy: -28, labelDx: -8 },
      { key: "utilization", label: "machine utilization", color: "var(--purple)", labelDy: 34, labelDx: 8 },
    ],
  });
